...
RETURN
  r.name AS role,
  collect(cl.name) AS accessible_classifications,
...
ORDER BY sensitivity_rank, node_type;
...
MATCH
  (r:Role {name: 'DATA_ANALYST'})-[:CAN_ACCESS]->
  (cl:Classification)<-[:HAS_CLASSIFICATION]-
  (n)
RETURN
  labels(n)[0] AS node_type,
...
MATCH
  (r:Role {name: 'DATA_ENGINEER'})-[:CAN_ACCESS]->
  (cl:Classification)<-[:HAS_CLASSIFICATION]-
  (n)
...
MATCH
  (r:Role {name: current_role})-[:CAN_ACCESS]->
  (cl:Classification)<-[:HAS_CLASSIFICATION]-
  (c:Customer)
RETURN
  c.customer_id,
  ...
  c.city,
  c.customer_tier,
  c.data_classification AS node_classification,
...
MATCH
  (r:Role {name: current_role})-[:CAN_ACCESS]->
  (cl:Classification)<-[:CLASSIFIED_AS]-
  (col:Column {table_name: 'EMPLOYEES'})
RETURN
...
MATCH
  path =
    (r:Role {name: 'DATA_GOVERNANCE_ADMIN'})-[:INHERITS_FROM*]->(child:Role)
...
OPTIONAL MATCH
  (r)-[:CAN_ACCESS]->(cl:Classification {name: requested_classification})
RETURN
  current_role AS role,
...
MATCH (col:Column)
WHERE
  col.data_classification IN ['Restricted', 'PII'] AND
  col.encryption_required = 'No'
RETURN
  col.table_name,
  col.name,
  col.data_classification,
  'COMPLIANCE RISK: Sensitive column not marked for encryption' AS warning;

// ── Data owner accountability ─────────────────────────────────────────────────
MATCH (col:Column)
WHERE col.data_owner IS NOT NULL AND col.data_owner <> ''
RETURN
  col.data_owner AS owner_team,
...
MATCH
  (r:Role {name: role_name})-[:CAN_ACCESS]->
  (cl:Classification)<-[:HAS_CLASSIFICATION]-
  (c:Customer {customer_id: 1001})-[:MADE_TRANSACTION]->
  (tx:Transaction)
RETURN
  role_name AS role,
